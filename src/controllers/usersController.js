...
const updateUser = async (req, res) => {
  ...
  const kycDocumentsUpload = createMulter("kyc_documents", "jpg").fields([
    { name: 'documentFront', maxCount: 1 },
    { name: 'documentBack', maxCount: 1 },
    { name: 'addressProof', maxCount: 1 },
  ]);

  // Handle profile picture upload and KYC documents upload
  profilePictureUpload(req, res, (err) => {
    ...
    kycDocumentsUpload(req, res, async (err) => {
      if (err) {
        console.error("KYC upload error:", err);
        return res.status(400).json({ message: "KYC document upload failed", error: err.message });
      }
      ...
      try {
        ...
        // Handle KYC documents (front, back, address proof)
        if (req.files) {
          if (req.files['documentFront'] && req.files['documentFront'][0].path) {
            user.kyc.documentFront = req.files['documentFront'][0].path;
          }
          if (req.files['documentBack'] && req.files['documentBack'][0].path) {
            user.kyc.documentBack = req.files['documentBack'][0].path;
          }
          if (req.files['addressProof'] && req.files['addressProof'][0].path) {
            user.kyc.addressProof = req.files['addressProof'][0].path;
          }
        }
